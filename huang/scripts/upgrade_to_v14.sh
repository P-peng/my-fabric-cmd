...
addCapabilityToChannel() {
        ...
        # Get the current channel config, decode and write it to config_updated.json
        fetchChannelConfig $CH_NAME config_updated.json

        # Modify the correct section of the config based on capabilities group
        if [ $GROUP == "orderer" ]; then
                jq -s '.[0] * {"channel_group":{"groups":{"Orderer": {"values": {"Capabilities": .[1].orderer}}}}}' config_updated.json ./scripts/capabilities.json > modified_config.json
        elif [ $GROUP == "channel" ]; then
                jq -s '.[0] * {"channel_group":{"values": {"Capabilities": .[1].channel}}}' config_updated.json ./scripts/capabilities.json > modified_config.json
        elif [ $GROUP == "application" ]; then
                jq -s '.[0] * {"channel_group":{"groups":{"Application": {"values": {"Capabilities": .[1].application}}}}}' config_updated.json ./scripts/capabilities.json > modified_config.json
        fi

        # Create a config updated for this channel based on the differences between config_updated.json and modified_config.json
        # write the output to config_update_in_envelope.pb
        createConfigUpdate "$CH_NAME" config_updated.json modified_config.json config_update_in_envelope.pb
